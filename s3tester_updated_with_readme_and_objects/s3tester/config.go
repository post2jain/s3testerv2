...
// Globals for enhanced features
var loadedObjectList []string
var globalKeyDistribution string
var globalConnLifetimeSec int
...
type Parameters struct {
	...
	ObjectListFile  string `json:"object-list-file,omitempty"`
	KeyDistribution string `json:"key-distribution,omitempty"`
	ConnLifetimeSec int    `json:"conn-lifetime-sec,omitempty"`
...
func parseByteSize(s string) (byteSize, error) {
	...
	if size, err := strconv.ParseInt(trimmed, 10, 64); err == nil {
		if size < 0 {
	...
	for _, unit := range units {
		if strings.HasSuffix(upper, unit.suffix) {
			valuePart := strings.TrimSpace(trimmed[:len(trimmed)-len(unit.suffix)])
			if valuePart == "" {
				return 0, fmt.Errorf("size missing value before %s", unit.suffix)
			}
			floatVal, err := strconv.ParseFloat(valuePart, 64)
			if err != nil {
				return 0, err
			}
			if floatVal < 0 {
				return 0, fmt.Errorf("size cannot be less than zero, got %v", floatVal)
			}
			bytes := floatVal * unit.multiplier
...
Note: Requests are generated in the same order that you specify operations. That is, if you specify a delete followed by a put, but have no existing objects to delete, all of the deletes will fail.`)
	...
	flags.StringVar(&params.ObjectListFile, "object-list-file", "", "Path to newline-separated object names to use for operations")
	flags.StringVar(&params.KeyDistribution, "key-distribution", "roundrobin", "Key distribution: roundrobin|random")
	flags.IntVar(&params.ConnLifetimeSec, "conn-lifetime-sec", 0, "Close/recycle connections after this many seconds (0 = disabled)")
	flags.IntVar(&params.TPS, "tps", 0, "Target transactions per second (overrides requests/duration calculation if >0)")
	flags.StringVar(&params.MetricsAddr, "metrics-addr", "", "Address for Prometheus metrics endpoint (e.g. :9090)")
	...
	if len(config.worklist) > 0 {
		config.Params = &config.worklist[0]
	} else {
		config.Params = params
	}

	// Setup enhanced globals
	globalKeyDistribution = params.KeyDistribution
	globalConnLifetimeSec = params.ConnLifetimeSec
	if params.ObjectListFile != "" {
		// load object list
		f, err := os.Open(params.ObjectListFile)
		if err != nil {
			return nil, fmt.Errorf("failed to open object list file: %v", err)
		}
		defer f.Close()
		...
		for s.Scan() {
			...
			loadedObjectList = append(loadedObjectList, s.Text())
		}
		if err := s.Err(); err != nil {
			return nil, fmt.Errorf("failed reading object list file: %v", err)
		}
	}
	return config, nil
}
